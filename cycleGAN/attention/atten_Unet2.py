...
def attention_block_2d(x, g, inter_channel, data_format='channels_last'):
    ...
    theta_x = Conv2D(inter_channel, [1, 1], strides=[1, 1], data_format=data_format,dilation_rate=(2, 2))(x)

    # phi_g(?,g_height,g_width,inter_channel)

    phi_g = Conv2D(inter_channel, [1, 1], strides=[1, 1], data_format=data_format,dilation_rate=(2, 2))(g)
    ...
    psi_f = Conv2D(1, [1, 1], strides=[1, 1], data_format=data_format,dilation_rate=(2, 2))(f)
...
def att_unet(img_w, img_h, data_format='channels_last'):
    ...
    for i in range(depth):
        x = Conv2D(features, 3, activation=LeakyReLU(), padding='same',dilation_rate=(2, 2))(x)
    ...
    x = Conv2D(features, (3, 3), activation=LeakyReLU(), padding='same',dilation_rate=(2, 2), data_format=data_format)(x)
    x = instancenormalization.InstanceNormalization()(x)
    x = Conv2D(features, (3, 3), activation=LeakyReLU(), padding='same',dilation_rate=(2, 2), data_format=data_format)(x)
    x = instancenormalization.InstanceNormalization()(x)
